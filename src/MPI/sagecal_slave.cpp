...
cout<<myrank<<" : "<<cm<<": downweight ratio ("<<iodata_vec[cm].fratio<<") based on flags."<<endl;
     ...
     /* precess source locations (also beam pointing) from J2000 to JAPP if we do any beam predictions,
      ...
      if (doBeam && !sources_precessed && (beam_vec[cm].elType!=ELEM_ALO)) {
  ...
  beam_vec[cm].bfType,beam_vec[cm].b_ra0,beam_vec[cm].b_dec0,beam_vec[cm].p_ra0,beam_vec[cm].p_dec0,iodata_vec[cm].freq0,beam_vec[cm].sx,beam_vec[cm].sy,beam_vec[cm].time_utc,iodata_vec[cm].tilesz,beam_vec[cm].Nelem,beam_vec[cm].xx,beam_vec[cm].yy,beam_vec[cm].zz,&elem_vec[cm],doBeam,Data::Nt);
	...
	/*MM: the first iteration you have to do this for all MS!*/
     ...
     if (Data::DoDiag) {
       ...
       beam_vec[cm].sy,beam_vec[cm].time_utc,beam_vec[cm].Nelem,beam_vec[cm].xx,beam_vec[cm].yy,beam_vec[cm].zz,&elem_vec[cm],doBeam,arho_vec[cm],Bpoly,Bii,Mt,Npoly,Nms,Data::Nt);
     } else {
     if (GPUpredict) {
