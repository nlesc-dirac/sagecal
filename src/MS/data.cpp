...
int Data::ccid=-99999;
int Data::ddid=-99999;
double Data::sp_gamma=1.0; /* -D option, contraint for applying spatial model onto diffuse model */
...
Data::loadDataList(vector<MSIter*> msitr, Data::IOData iodata, double *fratio) {
  ...
  for (int cm=0; cm<iodata.Nms;cm++) { 
    ...
    [[maybe_unused]] int n_corr=(*dataCols[0]).shape(0)[0];
...
Data::loadDataMinibatch(Table ti, Data::IOData iodata, int minibatch, double *fratio) {
    ...
    [[maybe_unused]] int n_corr=dataCol.shape(0)[0];
    // only support 4 correlations (TBD)
    assert(n_corr == 4);
