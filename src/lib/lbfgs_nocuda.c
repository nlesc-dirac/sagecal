...
func_grad00(
 ...
 for (ci=0; ci<m; ci++) {
    ...
    /* this parameter should correspond to the right baseline (x tilesz)
        to contribute to the derivative */
    cli=0;
    while((cli<dp->M) && (ci<dp->carr[cli].p[0] || ci>dp->carr[cli].p[0]+8*dp->N*dp->carr[cli].nchunk-1)) {
     cli++;
    }
   /* now either cli>=M: cluster not found 
       or cli<M and cli is the right cluster */
   if (cli==dp->M && ci>=dp->carr[cli-1].p[0] && ci<=dp->carr[cli-1].p[0]+8*dp->N*dp->carr[cli-1].nchunk-1) {
    cli--;
   }

   if (cli<dp->M) {
    /* right parameter offset */
    stci=ci-dp->carr[cli].p[0];
 
    stc=(stci%(8*dp->N))/8; /* 0..N-1 */
    /* make sure this baseline contribute to this parameter */
    tpchunk=stci/(8*dp->N);
    nchunk=dp->carr[cli].nchunk;
    pstart=dp->carr[cli].p[0];
    tilesperchunk=(dp->tilesz+nchunk-1)/nchunk;
    ...
    for (nb=0; nb<Nbase; ++nb) {
     ...
     sta1=dp->barr[nb].sta1;
     sta2=dp->barr[nb].sta2;
    
     if (((stc==sta1)||(stc==sta2))&& !dp->barr[nb].flag) {
      /* this baseline has a contribution */
      /* which paramter of this station */
      stoff=(stci%(8*dp->N))%8; /* 0..7 */
      /* which cluster */
      stm=cli; /* 0..M-1 */
     ...
     */
     /* read in residual vector, conjugated */
     xr[0]=(x[nb*8])-_Complex_I*(x[nb*8+1]);
     xr[1]=(x[nb*8+2])-_Complex_I*(x[nb*8+3]);
     xr[2]=(x[nb*8+4])-_Complex_I*(x[nb*8+5]);
     xr[3]=(x[nb*8+6])-_Complex_I*(x[nb*8+7]);

     /* read in coherency */
     C[0]=dp->coh[4*dp->M*nb+4*stm];
     C[1]=dp->coh[4*dp->M*nb+4*stm+1];
     C[2]=dp->coh[4*dp->M*nb+4*stm+2];
     C[3]=dp->coh[4*dp->M*nb+4*stm+3];
     ...
     if (stc==sta1) {
       ...
       G1[2]=pp[4]+_Complex_I*pp[5];
       G1[3]=pp[6]+_Complex_I*pp[7];
       poff=pstart+tpchunk*8*dp->N+sta2*8;
       G2[0]=(p[poff])+_Complex_I*(p[poff+1]);
       G2[1]=(p[poff+2])+_Complex_I*(p[poff+3]);
       G2[2]=(p[poff+4])+_Complex_I*(p[poff+4]);
       G2[3]=(p[poff+6])+_Complex_I*(p[poff+7]);
       ...
       G2[2]=pp[4]+_Complex_I*pp[5];
       G2[3]=pp[6]+_Complex_I*pp[7];
       poff=pstart+tpchunk*8*dp->N+sta1*8;
       G1[0]=(p[poff])+_Complex_I*(p[poff+1]);
       G1[1]=(p[poff+2])+_Complex_I*(p[poff+3]);
       G1[2]=(p[poff+4])+_Complex_I*(p[poff+5]);
       G1[3]=(p[poff+6])+_Complex_I*(p[poff+7]);
     }
