...
/*    stc=(ci%(8*N))/8;
  ...
  cudakernel_lbfgs_robust(dp->ThreadsPerBlock, dp->BlocksPerGrid, Nbase, dp->tilesz, M, N, Nparam, dp->g_start, dp->robust_nu, cxo, ccoh, cpp, cbb, cptoclus, cgrad);
...
/*****************************************************************************/
  ...
  while (ck<itmax) {
   ...
   /* update gradient */
   func_grad_robust_cuda(func,xk1,gk,x,m,n,step,gpu_threads,adata);
   /* yk=yk+gk1 */
   my_daxpy(m,gk,1.0,&y[cm]);
