...
class BeamGenerator:
    def __init__(
        ...
        self.phi_file_ = phi_file
        self.freq_file_ = freq_file
        self.ephi_file_ = ephi_file
        self.etheta_file_ = etheta_file
    ...
    def load_model(self):
        ...
        for fr in range(self.n_freq_):
            self.totalpow[fr] = np.sqrt(
                np.mean(np.abs(self.ephi_[fr, 0, :]) ** 2)
                + np.mean(np.abs(self.etheta_[fr, 0, :]) ** 2)
            )
    ...
    def setup_basis(self):
        ...
        for n in range(self.n0_):
            for m in range(-n, n + 1, 2):
                absm = abs(m)
                preamble[idx] = math.sqrt(
                    math.factorial((n - absm) // 2)
                    / math.factorial((n + absm) // 2)
                    / math.pi
                )
                if ((n - absm) // 2) % 2 == 1:
        ...
        self.basis_ = np.zeros(
            (self.nmodes_, self.horizon_idx, self.n_phi_), dtype=complex
        )
    ...
    def decompose_model_freq(self, freq, component="theta"):
        ...
        if component == "theta":
            ...
            err = np.linalg.norm(np.matmul(self.A_, x) - b_etheta) / np.linalg.norm(
                b_etheta
            )
        else:
            ...
            err = np.linalg.norm(np.matmul(self.A_, x) - b_ephi) / np.linalg.norm(
                b_ephi
            )
...
//This file is automatically generated\n"""
        ...
        self.out_fd_.write(
            "//modes=" + str(self.nmodes_) + " beta=" + str(self.beta_) + "\n"
        )
        ...
        for freq in range(self.n_freq_):
            ...
            for nm in range(self.nmodes_):
                self.out_fd_.write(
                    f" {np.real(x[nm]):e}+_Complex_I*({np.imag(x[nm]):e}),"
                )
            self.out_fd_.write("}," + "\n")
            if self.verbose:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate element beam coefficients for sagecal, input files used are theta.npy, phi.npy, frequency.npy, etheta.npy, and ephi.npy",
    ...
    parser.add_argument(
        "--show", action="store_true", default=False, help="show basis functions"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
